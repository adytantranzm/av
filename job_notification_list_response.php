...
date_default_timezone_set('Asia/Calcutta');
	...
	if( !empty($params['search']['value']) ) {
		...
		$where .=" OR CAST(Q.jobinfoid AS text) ILIKE '%".$paramsearchval."%' ";  
	...
	$sql = "SELECT Q.notifymasterid,concat(U.firstname,' ',U.lastname) as fullname,Q.jobinfoid,Q.title,Q.message,Q.url,Q.notifiedon,G.userid FROM notifymaster as Q LEFT OUTER JOIN gennotifyalloc as G ON Q.notifymasterid = G.notifymasterid JOIN userinfo as U ON G.userid = U.userid WHERE Q.jobinfoid is NOT NULL";
	$sqlTot .= $sql;
	$sqlRec .= $sql;
